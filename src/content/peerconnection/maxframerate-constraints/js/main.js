...
minFramerateInput.onchange = displayRangeValue;
maxFramerateInput.onchange = displayRangeValue;
...
function displayGetUserMediaConstraints() {
  ...
  console.log('[FPS_TEST] getUserMedia constraints', constraints);
  getUserMediaConstraintsDiv.textContent = JSON.stringify(constraints, null, '    ');
}

function displayEncodingParams(encodings) {
  const textOutput = JSON.stringify(encodings, null, 2);
  console.log('[FPS_TEST] Updating encoding parameters: ', textOutput);
  const encodingsParamOutput = document.getElementById('encodingParams');
  encodingsParamOutput.textContent = textOutput;
}
...
async function createPeerConnection() {
  ...
  console.log('[FPS_TEST] localPeerConnection creating offer');
  ...
  remotePeerConnection.ontrack = e => {
    if (remoteVideo.srcObject !== e.streams[0]) {
      console.log('[FPS_TEST] remotePeerConnection got stream');
  ...
  try {
    console.log('[FPS_TEST] localPeerConnection offering');
    const offer = await localPeerConnection.createOffer()
    localPeerConnection.setLocalDescription(offer);
    remotePeerConnection.setRemoteDescription(offer);
    
    console.log('[FPS_TEST] remotePeerConnection answering');
    const answer = await remotePeerConnection.createAnswer()
    remotePeerConnection.setLocalDescription(answer);
    localPeerConnection.setRemoteDescription(answer);

    // Applying transceiver encodings
    const videoTransceiver = localPeerConnection.getTransceivers().find((ts) => ts.mid === '1');
    const params = videoTransceiver.sender.getParameters();

    params.encodings = params.encodings.map(encoding => ({
      ...encoding,
      // Force a maxFrame of 10fps for all encodings
      maxFramerate: 10,
    }));

    displayEncodingParams(params.encodings);
...
function onAddIceCandidateError(error) {
  console.log(`[FPS_TEST] Failed to add Ice Candidate: ${error.toString()}`);
}
...
function dumpStats(results) {
  let statsString = '';
  results.forEach(res => {
    if (res.type !== 'outbound-rtp' && res.type !== 'inbound-rtp') {
      return;
    }
